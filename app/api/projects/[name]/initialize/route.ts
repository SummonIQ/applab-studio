...
`;
    ...
    // FIRST: Read existing config file to preserve apps that aren't on disk yet
    ...
    try {
      ...
      const appsMatch = existingConfig.match(/apps:\s*\[([\s\S]*?)\]\s*[,}]/);
      if (appsMatch) {
      ...
      console.log('[API] No existing config file to preserve');
    }

    // Analyze project to detect apps on disk
    ...
    // Merge: existing config apps + detected apps (detected takes priority for updates)
    ...
    // Default apps to create for new projects (only if no config AND no detected apps)
    const DEFAULT_APPS = [
      { name: 'web', type: 'web-app', portOffset: 0 },
      { name: 'api', type: 'api', portOffset: 1 },
      { name: 'marketing', type: 'marketing-site', portOffset: 2 },
      { name: 'desktop', type: 'desktop-app', portOffset: 3 },
    ];

    // Get project's base port for default apps
    let projectBasePort = 10000; // fallback
    const existingProject = await prisma.project.findUnique({
      where: { name },
      select: { port: true },
    });
    if (existingProject?.port) {
      projectBasePort = existingProject.port;
    }

    // Use merged apps, or create defaults if nothing exists
    const appsToUse =
      mergedApps.length > 0
        ? mergedApps
        : DEFAULT_APPS.map(app => ({
            ...
            type: app.type,
            path: `apps/${app.name}`,
            description: `${name} ${app.name} application`,
            devPort: projectBasePort + app.portOffset,
            devCommand: 'bun dev',
          }));

    // Generate apps array for config
    const appsConfigArray = appsToUse
      .map(
        app => `    {
      name: '${app.name}',
...
`;
    ...
    // Write config file (configPath already defined above)
    ...
    if (project && appsToUse.length > 0) {
      console.log('[API] Syncing', appsToUse.length, 'apps to database');
    ...
    const appCount = appsToUse.length;
    const wasDetected = detectedApps.length > 0;
    let message = 'SummonIQ config created';
    if (gitInitialized && appCount > 0) {
      message = wasDetected
        ? `SummonIQ config created with ${appCount} detected app${appCount > 1 ? 's' : ''} and Git repository initialized`
        : `SummonIQ config created with ${appCount} default app${appCount > 1 ? 's' : ''} and Git repository initialized`;
    } else if (gitInitialized) {
      message = 'SummonIQ config created and Git repository initialized';
    } else if (appCount > 0) {
      message = wasDetected
        ? `SummonIQ config created with ${appCount} detected app${appCount > 1 ? 's' : ''}`
        : `SummonIQ config created with ${appCount} default app${appCount > 1 ? 's' : ''}`;
    }
