...
export function FileBrowserContent() {
  ...
  const [activeTab, setActiveTab] = useState<'files' | 'tree'>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('files-panel-activeTab');
      return (saved as 'files' | 'tree') || 'files';
    }
    return 'files';
  });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('files-panel-activeTab', activeTab);
    }
  }, [activeTab]);
