...
export class MCPClient extends EventEmitter {
  ...
  async connect() {
    ...
    // Get MCP server path from environment variable - must be set at runtime
    const mcpServerPath = process.env.MCP_SERVER_PATH;

    if (!mcpServerPath) {
      throw new Error(
        'MCP_SERVER_PATH environment variable is not set. Please set it to the path of the MCP server.',
      );
    }
