...
export function Canvas({
  ...
  useEffect(() => {
    // Reset pan to a sane, clamped center whenever the project/page changes.
    // The old hard-coded negative offsets were pushing the canvas off-screen,
    // which prevented drop events from landing inside the canvas.
    setPan(constrainPan({ x: 0, y: 0 }));
  }, [project?.id, project?.currentPageId, constrainPan]);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    e.stopPropagation();
    setIsDragging(false);
    // Native DnD can drop custom MIME types inconsistently (especially in Electron).
    // Prefer dataTransfer, but fall back to text/plain and finally the store.
    const rawType =
      e.dataTransfer.getData('componentType') ||
      e.dataTransfer.getData('text/plain');
    const componentType = (rawType || draggedComponentType) as
      | ComponentType
      | null;
    ...
    // Always clear drag payloads after a drop attempt so we don't reuse stale data.
    setDraggedComponentType(null);
    setDraggedAiComponentData(null);

    if ((aiComponentDataString || draggedAiComponentData) && project && rootId) {
      try {
        const componentData = aiComponentDataString
          ? JSON.parse(aiComponentDataString)
          : draggedAiComponentData;
  ...
  // Allow dropping anywhere in the workspace (not just the canvas element) so users
  // don't have to find the exact canvas bounds after layout shifts.
  const handleWorkspaceDrop = (e: React.DragEvent) => {
    ...
    if (!state.project || !root) {
      console.warn('Workspace drop blocked - missing project or root');
      return;
    }
    ...
    const rawType =
      e.dataTransfer.getData('componentType') ||
      e.dataTransfer.getData('text/plain') ||
      state.draggedComponentType;
    const aiComponentDataString =
      e.dataTransfer.getData('aiComponentData') ?? null;
    ...
    if (aiComponentDataString) {
      try {
        ...
        state.insertComponentWithData(componentData, root, 'inside');
        return;
      } catch (error) {
        console.error('Failed to parse AI component data', error);
      }
    } else if (state.draggedAiComponentData) {
      state.insertComponentWithData(state.draggedAiComponentData, root, 'inside');
      return;
    }

    if (rawType) {
      state.addComponent(rawType as ComponentType, root);
    }
  };
    ...
    <div className="h-full flex flex-col" data-canvas-theme={canvasTheme}>
      ...
      {viewMode === 'run' ? (
        ...
        >
          ...
          onDragEnter={handleWorkspaceDragOver}
          ...
          >
            ...
            <div
              ref={canvasRootRef}
              className={cn(
                'bg-background rounded-md shadow-xl transition-all duration-150 border border-border/60',
                isDragging
