...
export function ToolboxContent({ onDragEnter }: ToolboxContentProps) {
  ...
  const globalDragOverListenerRef = useRef<((e: DragEvent) => void) | null>(
    null,
  );

  const startGlobalDragTracking = () => {
    if (typeof window === 'undefined') return;
    if (globalDragOverListenerRef.current) return;

    const listener = (ev: DragEvent) => {
      lastDragPointRef.current = { x: ev.clientX, y: ev.clientY };
    };

    globalDragOverListenerRef.current = listener;
    window.addEventListener('dragover', listener, true);
  };

  const stopGlobalDragTracking = () => {
    if (typeof window === 'undefined') return;
    const listener = globalDragOverListenerRef.current;
    if (!listener) return;
    window.removeEventListener('dragover', listener, true);
    globalDragOverListenerRef.current = null;
  };
  ...
  const [activeTab, setActiveTab] = useState<
    'components' | 'generate' | 'chat'
  >(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('toolbox-activeTab');
      return (
        (saved as 'components' | 'generate' | 'chat') || 'components'
      );
    }
    return 'components';
  });
  ...
  const handleDragStart = (e: React.DragEvent, type: ComponentType) => {
    ...
    // Clear any previous AI drag payload so drops don't accidentally reuse it.
  ...
  const handleAiDragStart = (e: React.DragEvent, component: any) => {
    ...
    e.dataTransfer.effectAllowed = 'copy';
    setIsDragging(true);
    // Electron/native DnD can lose dataTransfer payloads; keep a store fallback.
    setDraggedComponentType(component.type);
    setDraggedAiComponentData(component);
  };

  const handleDragEnd = (e: React.DragEvent) => {
    // Fallback for environments where native drop/mouseup doesn't reliably fire (Electron/WebKit).
    // If the drag ends over the canvas and we're still marked as dragging, insert the item here.
    const state = useBuilderStore.getState();
    const point = lastDragPointRef.current ?? { x: e.clientX, y: e.clientY };
    ...
    const root = state.getRootId();
    if (!root) {
      cleanup();
      return;
    }

    const isContainerType = (type: unknown): boolean => {
      return (
        type === 'Page' ||
        type === 'Container' ||
        type === 'Flex' ||
    ...
    const pickDropTargetId = (): string | null => {
      // Prefer the exact component under the cursor (deepest), but walk up until we find a container.
      ...
      while (cursor) {
        const compHost = cursor.closest?.('[data-component-id]') as HTMLElement | null;
        if (!compHost) break;

        const id = compHost.getAttribute('data-component-id');
        if (id && components[id] && isContainerType(components[id].type)) {
          return id;
        }

        // Move up and keep searching.
        cursor = compHost.parentElement;
      }

      // If nothing else, allow dropping onto the root canvas container.
    ...
    try {
      if (state.draggedAiComponentData) {
        state.insertComponentWithData(state.draggedAiComponentData, targetId, 'inside');
      } else if (state.draggedComponentType) {
        // Use insertComponent so container drop behavior matches other drop paths.
        state.insertComponent(state.draggedComponentType, targetId, 'inside');
      }
    } finally {
      cleanup();
    }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        {activeTab === 'components' ? (
          ...
          <AIPanelContent tab="generate" showTabs={false} />
        ) : (
          <AIPanelContent tab="chat" showTabs={false} />
        )}
      </div>
    </div>
  );
