...
export function PropertiesPanel() {
  ...
  const renderLayoutControls = () => {
    ...
    const rawJustifyContent = selectedComponent.styles.justifyContent || 'start';
    // `justify-content` doesn't support `stretch` in flexbox; treat as start.
    const justifyContent =
      rawJustifyContent === 'stretch' ? 'start' : rawJustifyContent;
...
export function PropertiesPanelContent() {
  ...
  const directChildren = !isMultiSelect && selectedComponent
    ? selectedComponent.children
        .map(child => components[child.id] || child)
        .filter(Boolean)
    : [];

  const justifyLikelyNoop =
    !isMultiSelect &&
    !!selectedComponent &&
    directChildren.length > 0 &&
    directChildren.every(child => {
      const styles: any = (child as any).styles ?? {};
      const fillsViaFlex =
        styles.flex === '1' || styles.flex === 1 || !!styles.flexGrow;
      const fillsViaExplicitSize =
        resolvedFlexDirection === 'column'
          ? styles.height === '100%'
          : styles.width === '100%';
      return fillsViaFlex || fillsViaExplicitSize;
    });
  ...
  // Hooks must be declared unconditionally (even when nothing is selected),
  // otherwise React will detect a hook-order mismatch when selection changes.
  ...
  // Reset edit mode when selection changes (or when multi-select activates).
  useEffect(() => {
          ...
          <CollapsibleContent>
            <div className="p-3 space-y-3 border-b border-border">
              ...
              <div>
                ...
                {!isMultiSelect && selectedComponent.children.length === 0 && (
                  <p className="text-[10px] text-muted-foreground/70 mb-1">
                    Add children to see justify affect spacing along the main axis.
                  </p>
                )}
                {!isMultiSelect &&
                  ...
                  justifyLikelyNoop && (
                    <p className="text-[10px] text-muted-foreground/70 mb-1">
                      Justify may look like it’s doing nothing because the children
                      are filling the available space (e.g. <span className="font-medium">flex: 1</span>).
                      Reduce child flex/size to see distribution.
                    </p>
                  )}
