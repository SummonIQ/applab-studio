...
export function HeroSpiralCanvas({ mouse, variant = 1 }: Props) {
  ...
  useEffect(() => {
    ...
    material.uniforms.arms.value = preset.arms;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          float twist = ang * arms + r * 7.0 - t * 3.2;
          float band = 0.5 + 0.5 * sin(twist);
          band = pow(band, 1.6);

          float dust = hash(floor(p*10.0 + t));
          float sparkle = step(0.97, dust) * (0.5 + 0.5*sin(t*5.0 + dust*6.2831));

          float core = smoothstep(0.55, 0.0, r);

          float g = band * smoothstep(1.2, 0.15, r);
          g = max(g, core * 0.55);
          g += 0.25 * sparkle * smoothstep(1.0, 0.2, r);

          vec3 col = mix(colorB, colorA, g);

          float edge = smoothstep(1.25, 0.35, length((uv - 0.5) * vec2(2.2,1.6)));
