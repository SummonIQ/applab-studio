...
function findNodeModulesDir(startDir, stopDir) {
  try {
    ...
    while (true) {
      ...
      // Stop after checking stopDir, not before
      ...
      submenu: [
        {
          ...
          click: () => {
            ...
            if (mainWindow) {
              mainWindow.loadURL(
                isDev
                  ? 'http://localhost:30020/projects'
                  : `file://${path.join(__dirname, '../.next/projects/index.html')}`,
              );
            }
          },
        },
        {
          ...
          click: async () => {
            await shell.openExternal(
              'https://github.com/bright-and-early/summoniq/issues',
            );
          },
        },
      ],
