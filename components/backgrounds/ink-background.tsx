...
export function HeroInkCanvas({ mouse, variant = 1 }: Props) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          float n2 = fbm(q + vec2(-t*0.25, t*0.18));

          vec2 adv = flow * 0.55 * vec2(n1 - 0.5, n2 - 0.5);
          adv += 0.22 * m;

          float ink = fbm(q + adv * 2.0);
          float veins = smoothstep(0.25, 0.95, ink) * (0.6 + 0.4 * sin((p.x - p.y) * 2.2 + t * 1.1));

          vec3 col = mix(colorB, colorA, veins);
          ...
          float aout = alpha * vignette * edgeFade * (0.18 + 0.82 * veins);
