...
export class FilesystemSyncService {
  private static hasWarnedMissingSyncQueue = false;
  /**
  ...
  static async processSyncQueue(batchSize = 10) {
    const syncQueue = (
      db as {
        syncQueue?: { findMany: (args: unknown) => Promise<SyncQueue[]> };
      }
    ).syncQueue;

    if (!syncQueue) {
      if (!this.hasWarnedMissingSyncQueue) {
        log(
          'SyncQueue model not available on Prisma client; skipping sync processing.',
        );
        this.hasWarnedMissingSyncQueue = true;
      }
      return;
    }

    // Get pending items
    const items = await syncQueue.findMany({
      where: {
        status: 'PENDING',
        attempts: {
          lt: 3, // Max 3 attempts
  ...
  private static async syncProjectConfig(item: SyncQueue) {
    ...
    const fileContent = this.formatConfigContent(
      config.configType,
      config.content,
    );
  ...
  private static async syncDependency(item: SyncQueue) {
    ...
    if (Object.keys(deps.optionalDependencies).length > 0) {
      packageJson.optionalDependencies = this.sortObject(
   ...
   */
  private static async getProjectIdFromPath(
    filePath: string,
   ...
   */
  private static async getAppIdFromPath(
    filePath: string,
  ): Promise<string | undefined> {
